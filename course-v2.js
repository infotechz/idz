...
const lesson = (moduleId, id, title, description, minutes, practicalExample, exerciseTitles) => ({
  ...
  video: "", pdfUrl: "", imgUrl: "", estimatedMinutes: minutes,
  exercises: exerciseTitles.map((exerciseTitle, index) => ({ id: `${id}-ex${index + 1}`, title: exerciseTitle, question: `Como você aplicaria o que aprendeu em ${title.toLowerCase()}?`, options: ["Praticando e conferindo cada etapa", "Ignorando o resultado", "Compartilhando senhas", "Pulando a atividade"], correctAnswer: 0, explanation: "A prática guiada e a conferência ajudam a consolidar o aprendizado.", difficulty: index ? "medio" : "facil", mandatory: true, estimatedMinutes: 5 }))
});
...
const examples=["Organizar uma tarefa cotidiana no computador.","Repetir a atividade com um arquivo de exemplo.","Conferir o resultado antes de salvar.","Resolver um pequeno desafio guiado."];
const courseV2Data={courseVersion:COURSE_VERSION,title:"INFORMÁTICA DO ZERO",welcome:{id:"welcome",type:"welcome",title:"Boas-vindas",description:"Conheça a plataforma e comece sua jornada."},modules:moduleDefinitions.map(([moduleId,title,titles])=>({id:moduleId,moduleId,type:moduleId==="m12"?"project":"lesson",title,lessons:titles.map((lessonTitle,index)=>lesson(moduleId,`${moduleId}-a${index+1}`,lessonTitle,`Conteúdo prático sobre ${lessonTitle.toLowerCase()}, com orientação passo a passo.`,moduleId==="m12"?30:15,examples[index%examples.length],index===titles.length-1?["Concluir a atividade","Conferir o resultado"]:["Praticar o conceito","Identificar a aplicação"]))})),finalProject:{id:"final-project",type:"project",title:"Projeto Final",steps:["Estrutura de pastas","Currículo","Planilha financeira","Apresentação","E-mail com anexo","Arquivos no Drive","Exercício de segurança"].map((title,index)=>({id:`final-${index+1}`,title,mandatory:true,completed:false}))},bonuses:[{id:"bonus-1",type:"bonus",title:"🎁 Bônus 1 — 30 atalhos de teclado",downloadUrl:""},{id:"bonus-2",type:"bonus",title:"🎁 Bônus 2 — Modelo de currículo editável",downloadUrl:"",fallback:"Material em preparação"},{id:"bonus-3",type:"bonus",title:"🎁 Bônus 3 — Planilha financeira",downloadUrl:"",fallback:"Material em preparação"}]};
if(typeof window!=="undefined")window.IDZ_COURSE_V2=courseV2Data;
/* FASE 20A: carregador exclusivamente visual/estrutural. */
if(typeof window!=="undefined")window.addEventListener('DOMContentLoaded',()=>{if(document.querySelector('script[data-idz20a-loader]'))return;const s=document.createElement('script');s.src='fase20a-redesign.js?v=20a-definitiva';s.defer=true;s.dataset.idz20aLoader='1';document.body.appendChild(s)},{once:true});