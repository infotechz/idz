...
function navigationForPresentation(state){
  ...
  const student=[['house','Início','dashboard'],['book-open','Meu curso','lessons'],['diagram-project','Projeto Final','project'],['award','Certificado','certificate'],['gift','Bônus','bonuses'],['headset','Ajuda','support'],['user','Minha conta','profile']];
  ...
  const entries=(state==='ADMIN'?admin:student).map(([i,label,action])=>({icon:'fa-'+i,label,action}));
  if(state==='ADMIN'){
    const item=action=>entries.find(x=>x.action===action);
    return [{...item('admin'),label:'Visão geral'},item('admin-students'),{label:'Curso',children:['admin-content','admin-exercises','admin-project','lessons'].map(item)},{label:'Financeiro',children:['admin-finance','admin-cart','admin-coupons','admin-refunds'].map(item)},{label:'Atendimento',children:['admin-support','admin-notifications'].map(item)},item('admin-certificates'),item('settings'),{icon:'fa-right-from-bracket',label:'Sair',action:'logout',danger:true}];
  }
  return [...entries,{icon:'fa-right-from-bracket',label:'Sair',action:'logout',danger:true}];
}
...
function renderStudentDashboard(){
  ...
  const metrics=[['Progresso geral',stats.percent+'%'],['Aulas',stats.completedLessonCount+'/'+stats.totalLessons],['Exercícios',stats.completedExerciseCount+'/'+stats.totalExercises]];
  host.innerHTML=`${resume?`<article class="card continue-card"><div><span class="eyebrow">CONTINUE DE ONDE PAROU</span><h2>${escapeHTML(resume.mod.title)}</h2><p>${escapeHTML(resume.lesson.title)}</p>${progressBar(stats.percent)}<button class="btn" onclick="showStudentPage('lessons')">Continuar estudando ${icon('arrow-right')}</button></div><img src="assets/hero/idz-laptop.webp" alt="Seu curso IDZ" width="1200" height="800" loading="lazy"></article>`:'<div class="empty-state">As aulas estarão disponíveis em breve.</div>'}<div class="stat-grid">${metrics.map(([label,val])=>`<article class="card stat-card"><small>${label}</small><strong>${val}</strong>${progressBar(label==='Progresso geral'?stats.percent:label==='Módulos'?Math.round(stats.completedModuleCount/courseData.length*100):label==='Aulas'?Math.round(stats.completedLessonCount/Math.max(1,stats.totalLessons)*100):Math.round(stats.completedExerciseCount/Math.max(1,stats.totalExercises)*100))}</article>`).join('')}</div><div class="quick-grid"><button class="card quick-card" onclick="showStudentPage('project')"><span class="icon-tile">${icon('diagram-project')}</span><span><strong>Projeto Final</strong><small>${stats.completedProjectStepCount}/${stats.totalProjectSteps} etapas concluídas</small></span></button><button class="card quick-card" onclick="showStudentPage('certificate')"><span class="icon-tile">${icon('award')}</span><span><strong>Certificado</strong><small>${certificateAvailable()?'Disponível':'Complete os requisitos'}</small></span></button></div>`;
}
...
function showCourseExercise(modId,lessonId){selectedModuleId=String(modId);studentView='exercises';activeLessonTab='exercises';loadLessonContent(modId,lessonId);}
function renderStudentView(view='dashboard'){
  ...
  document.getElementById('student-detail').hidden=view==='dashboard'||courseView;
  document.getElementById('course-primary-tabs').hidden=!courseView;
...
function renderLessonPresentation(data){
  ...
  selectedModuleId=String(mod.id);
  localStorage.setItem('idz_resume_'+progressKey(),JSON.stringify(activeLesson));
  if(studentView==='exercises'){
    document.getElementById('student-exercises-view').innerHTML=`<button class="btn-outline" onclick="renderExercisesView()">Voltar aos exercícios</button><h2 class="lesson-heading">${escapeHTML(lesson.title)}</h2>${quizHtml||'<p>Nenhum exercício nesta aula.</p>'}`;
    document.querySelectorAll('#student-exercises-view .activity-panel').forEach(el=>el.classList.add('is-open'));
    document.querySelectorAll('#student-exercises-view .exercise-toggle').forEach(el=>el.remove());
    return;
  }
  ...
  document.getElementById('lms-main-content').innerHTML=`${mediaHtml||'<div class="card empty-state"><i class="fa-solid fa-book-open"></i><h3>Aula em texto</h3><p>Estude o conteúdo e pratique nos exercícios abaixo.</p></div>'}<header class="lesson-heading"><small>${escapeHTML(mod.title)}</small><h2>${escapeHTML(lesson.title)}</h2></header><div style="margin:18px 0">${progressBar(progressStats.percent)}</div><div class="lesson-actions"><button class="btn-outline" ${previousLesson?'':'disabled'} ${previousLesson?`onclick="loadLessonContent('${previousLesson.modId}','${previousLesson.lessonId}')"`:''}>${icon('arrow-left')} Aula anterior</button><button class="btn" ${lessonDone?'disabled':''} onclick="markLessonAsDone(${mod.id},${lesson.id})">${icon('check')} ${lessonDone?'Aula concluída':'Concluir aula'}</button><button class="btn" ${nextLesson?'':'disabled'} ${nextLesson?`onclick="loadLessonContent('${nextLesson.modId}','${nextLesson.lessonId}')"`:''}>Próxima aula ${icon('arrow-right')}</button></div><div class="tabs lesson-tabs" role="tablist" aria-label="Conteúdo da aula">${[['content','Conteúdo'],['exercises','Exercício'],['materials','Materiais']].map(([id,label])=>`<button class="tab" id="tab-${id}" role="tab" aria-controls="panel-${id}" data-lesson-tab="${id}" onclick="setLessonTab('${id}')">${label}</button>`).join('')}</div><section class="card lesson-panel" id="panel-content" role="tabpanel" aria-labelledby="tab-content" data-lesson-panel="content"><details class="lesson-summary"><summary>Sobre esta aula</summary><p>${escapeHTML(lesson.bloco1||lesson.introduction||'')}</p><p>${escapeHTML(lesson.bloco2||lesson.description||'')}</p><p>${escapeHTML(lesson.bloco3||(lesson.objectives||[]).join(' · '))}</p></details>${imgHtml}${finalProjectHtml}</section><section id="panel-exercises" role="tabpanel" aria-labelledby="tab-exercises" data-lesson-panel="exercises">${quizHtml||'<div class="card empty-state">Esta aula não possui exercícios.</div>'}<div class="lesson-actions"><button class="btn-outline" onclick="nextExercise()">Próximo exercício ${icon('arrow-right')}</button></div></section><section class="card lesson-panel" id="panel-materials" role="tabpanel" aria-labelledby="tab-materials" data-lesson-panel="materials">${pdfHtml||'<h3>Materiais da aula</h3><p>Nenhum arquivo complementar disponível nesta aula.</p>'}</section>${certificateSectionHtml}`;
